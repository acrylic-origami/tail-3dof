...
static uint8_t populate(int32_t* vs, int32_t* ts, int32_t* xbnd, joint_phys_t *phys, int32_t t_default) {
	volatile int32_t a = ts[0], b = ts[1];
	for(uint8_t i = 0; i < 2; i++) {
		volatile int32_t c = vs[i * 2], d = vs[i * 2 + 1];
		volatile int32_t xs[2] = { -1, -1 };
		uint8_t corners[2] = { abs(b) < EPS, abs(d) < EPS };
		if(corners[0] && corners[1]) {
			// REALLY degenerate case: a < T0, forall t. fallback to default time (nominally alpha/v)
			if(abs(a) < phys->T0)
				xs[1] = t_default;
			else
				return 1;
			...
			volatile int32_t T0_ceil = phys->T0 * ((1 << _W) - (abs(c) << _W) / phys->SMAX) >> _W;
			if(abs_quad(a, b, T0_ceil, xs))
				return 1;
		}
		else if(corners[0]) {
			// instead of doing the |a| < T0(1-|c-dx|/SMAX), do via quadratic and hijack the machinery that we have already
			// pc[0] = d * d / phys->SMAX / phys->SMAX;
			// volatile int32_t c0 = (1 - c / phys->SMAX);
			// pc[1] = -2 * d * c0 / phys->SMAX;
			// pc[2] = c0 * c0 - a * a / phys->T0 / phys->T0;

			volatile int32_t s = phys->SMAX * ((1 << _W) - (a << _W) / phys->T0) >> _W;
			if(s < 0) { // never intersects
				return 1;
			}
			volatile int32_t xs_[2] = { ((-s - c) << _W) / abs(d), ((s - c) << _W) / abs(d) };
//			volatile int32_t xs__[2] = { min(xs_[0], xs_[1]), min(xs_[0], xs_[1]) };
			cbnd(xs, xs_);
		}
		else {
			...
			for(i = 0; i < TIM_ITER_LIM; i++) {
				volatile int32_t dist = origin_dist_sq(a_, b_, c_, x);
				volatile int32_t delta = dist - R2;
				if(abs(delta) < TIM_ITER_EPS)
					break;

				volatile int32_t df = origin_dist_sq_p(a_, b_, c_, x);
				volatile int32_t delta_x = delta / (df >> _W); // head away if inside
				if(delta_x == 0) {
					delta_x = sgn(delta) * sgn(df);
				}
				x -= delta_x;
			}
			if(i == TIM_ITER_LIM)
...
uint8_t traj_t(int32_t x0, int32_t x1, int32_t v0, int32_t v1, int32_t *bnd, const joint_phys_t *phys) {
	int32_t vm = x1 - x0;
	int32_t vtf[2] = { abs((v0 << _W)/ALPHA), abs((v1 << _W)/ALPHA) }; // note: _reciprocal_ of time
	...
	if(v0 < EPS) {
		// special-case the starting torque from zero-velocity endpoint (shift the balance from optimistic to pessimistic)
		lt[1] = (6 * phys->I >> _W) * vm; // UNFLOAT 6.0
	}
	...
	if(v1 < EPS) {
		rt[1] = (6 * phys->I >> _W) * vm; // UNFLOAT 6.0
	}
	bnd[0] = -1; bnd[1] = -1;

	return populate(lv, lt, bnd, phys, v0 == 0 ? MAX_T_DEFAULT : (ALPHA << _W) / v0)
		|| populate(rv, rt, bnd, phys, v1 == 0 ? MAX_T_DEFAULT : (ALPHA << _W) / v1);
}
void lerp(int32_t a, int32_t b, int32_t av, int32_t bv, int32_t tf, int32_t t, int16_t *tr) {
	int32_t av_ = abs(av); // max(abs(av), (1 << _TW) / tf);
	int32_t bv_ = abs(bv); // max(abs(bv), (1 << _TW) / tf);

	int32_t l = ((av * t >> _W) + a), lp = av;
	int32_t m = (b - a) * t / tf + a, mp = (b - a << _W) / tf; // EXEMPT: * followed by /
	int32_t r = ((bv * (t - tf) >> _W) + b), rp = bv;
	if((av_ == 0 || t < (ALPHA << _W) / av_) && (bv_ == 0 || t >= (tf - (ALPHA << _W) / bv_))) {
		int32_t t0 = t * av_ / ALPHA, t1 = (tf - t) * bv_ / ALPHA, t2 = (t << _W) / tf;
		int32_t s0 = smoothstep(t0), s1 = smoothstep(t1);
		int32_t s0p = (smoothstep_p(t0) << _W) / tf, s1p = (smoothstep_p(t1) << _W) / tf;
		int32_t s2 = smoothstep(t2), s2p = (smoothstep_p(t2) << _W) / tf;
		tr[0] =
			(
				(l * ((1 << _W) - s0) + m * s0 >> _W)
				* ((1 << _W) - s2)
